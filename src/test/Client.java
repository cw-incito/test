...
public class Client {
	...
	public static void main(String[] args) throws Exception {
		...
		// 一次性验证某邀请码
		// 处理邀请码
		List<String> list1 = createInvcode("0529eb#9a1@a342e");
		// 注册账号
		regis(list1);
...
//		}
		...
		while (!urlSet.isEmpty()) {
			...
			for (Iterator<String> i = urlSet.iterator(); i.hasNext();) {
				...
				witerAddress(str);
				contentHttpGet.setURI(new URI("http://" + url + "/" + str));
				...
				System.out.println("connection-->http://" + url + "/" + str);
	...
	static String url = "wo.yao.cl";
	static HttpPost httpPost = new HttpPost("http://" + url + "/register.php");
	static HttpGet urlHttpGet = new HttpGet("http://" + url
			+ "/thread0806.php?fid=7&search=1");
	...
	private static Set queryUrls() throws ClientProtocolException, IOException,
	...
	private static void regis(List<String> list)
	...
	private static UrlEncodedFormEntity setPatams(String invcode)
			throws ClientProtocolException, IOException {

		List<NameValuePair> nvps = new ArrayList<NameValuePair>();

		nvps.add(new BasicNameValuePair("regname", "星期四"));
		nvps.add(new BasicNameValuePair("regpwd", "124254701"));
		nvps.add(new BasicNameValuePair("regpwdrepeat", "124254701"));
		nvps.add(new BasicNameValuePair("regemail", "youxi760@163.com"));
		...
		for (int i = 0; i < strs.length; i++) {
			...
			if (str.indexOf("#") >= 0
				...
				for (int j = 0; j < 10; j++) {
					...
					&& str.indexOf("#") == str.lastIndexOf("#")
					&& str.indexOf("@") >= 0
					&& str.indexOf("@") == str.lastIndexOf("@")) { // 存在一个#和一个@
